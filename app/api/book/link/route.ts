...
/** POST { email, listingUrl, civility? } -> lien de réservation client + envoi du mail d'invitation. */
export async function POST(req: Request) {
  const auth = getAuth(req);
  if (!auth) return NextResponse.json({ error: "Non connecté." }, { status: 401 });

  try {
    const { email, listingUrl, civility } = (await req.json()) as {
      email?: string; listingUrl?: string; civility?: string;
    };
    ...
    return NextResponse.json({ ok: true, bookUrl, emailSent });
  } catch (e) {
