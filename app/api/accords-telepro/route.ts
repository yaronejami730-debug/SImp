 ...
 *  Admin : tous. Téléprospecteur : uniquement les siens (il en est le bénéficiaire). */
export async function GET(req: Request) {
  ...
  try {
    const { rows } = await getPool().query(
      `select r.id, r.commercial_email, r.payee_email, r.base_eur, r.pct_nego, r.sold_eur, r.sold_pct, r.sold_pct_base, r.trigger_kind, r.label,
        ...
        where r.payee_kind = 'telepro' and r.call_center_id is null and r.active
          ${s.role === "admin" ? "" : "and lower(r.payee_email) = lower($1)"}
        order by r.id desc`,
      s.role === "admin" ? [] : [s.email],
    );
 ...
 *  Admin : pour n'importe quel téléprospecteur. Téléprospecteur : uniquement pour lui-même
 *  (teleproEmail envoyé ignoré, forcé à son propre e-mail — jamais au nom d'un autre). */
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!b.commercialEmail?.trim() || !teleproEmail?.trim()) {
    ...
    const { rows } = await pool.query(
      `insert into remuneration_accords (call_center_id, commercial_email, payee_email, payee_kind, base_eur, pct_nego, sold_eur, sold_pct, sold_pct_base, trigger_kind, payer_email, label)
       values (null, $1, $2, 'telepro', $3, $4, $5, $6, $7, $8, $1, $9) returning id`,
      [payer, payee, base, pct, soldEur, soldPct, soldPctBase, trig, (b.label ?? "").trim() || "Accord direct avec un téléprospecteur indépendant"],
    );
